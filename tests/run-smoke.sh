...
run_and_check_success_output_contains() {
    local name="$1"
    local expected="$2"
    shift 2
    ...
    if [[ "$status" != 0 || "$output" != *"$expected"* ]]; then
        printf 'FAIL %s\nstatus: %s\nexpected success with output containing:\n%s\nactual output:\n%s\n' "$name" "$status" "$expected" "$output" >&2
        return 1
    fi
    printf 'PASS %s\n' "$name"
}
...
run_and_check_malloc_tracer() {
    ...
    set +e
    "$emulator" --malloc-trace /tmp/lue-malloc-tracer >"$stdout_file" 2>"$stderr_file"
    status=$?
    set -e
    ...
    if [[ "$status" != 0 || "$stdout" != "malloc tracer guest done" ||
        "$stderr" != *"malloc-tracer ERROR: heap buffer overflow write"* ||
        "$stderr" != *"malloc-tracer ERROR: use-after-free write"* ||
        "$stderr" != *"leak(s) detected"* ]]; then
        printf 'FAIL %s\nstatus: %s\nstdout:\n%s\nstderr:\n%s\n' "$name" "$status" "$stdout" "$stderr" >&2
        return 1
    fi
    printf 'PASS %s\n' "$name"
}

run_and_check_contains help "--malloc-trace" "$emulator" --help
